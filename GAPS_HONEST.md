...
النسبة الحقيقية مقارنة بـ Windows GM82 الكامل: **~46% (أقل من 50%)**
...
| **GML Runtime / Interpreter** | ~48% | دعم التعابير، الشرطية، الحلقات (`while`, `repeat`, `do...until`), الهياكل (`ds_list`, `ds_map`), دال السلاسل والمطبوعات |
| **DnD Actions Engine** | ~42% | دعم الحركة، تغيير الكائن، تغيير السبرايت، المنبهات |
| **Physics & Collisions** | ~38% | AABB + Tile Platforms + bbox_* variables؛ لا يوجد Precise per-pixel collision mask |
...
| **النسبة الكلية** | **~46%** | **أقل من 50% مقارنة بنواة ويندوز GM82 الكاملة** |
...
5. حلقات التحكم والتكرار ومكتبات GML البرمجية (`while`, `do...until`, `string_pos`, `string_char_at`, `lerp`, `clamp`).
...
عدم ادعاء "100%" أو "Complete Engine". النسبة الحالية هي **46%** حقيقية مع التوسع المستمر الصادق.
